  ...
  const onSubmit = async (data: any) => {
    try {
      const response = await axios.post(
        "http://localhost:3000/api/v1/admin/change-password",
        {
          id: user.id,
          ...
          <div className="mt-5">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              {errors.cnfNewPassword && (
                <ErrorMessage text={errors.cnfNewPassword.message || ""} />
              )}
              <div className="flex justify-between">
                <Link
                  className="submit-button  mt-4  text-black py-2 px-4 rounded border border-black text-sm font-medium font-sans"
                  href={"/settings"}
                >
                  Back
                </Link>
